package org.hzero.platform.infra.remote.api.impl;
...
public class RemoteCustomizeServiceImpl implements RemoteCustomizeService {
    ...
    private ParameterizedTypeReference<List<MethodMetaData>> METHOD_TYPE = new ParameterizedTypeReference<List<MethodMetaData>>() {};

    public RemoteCustomizeServiceImpl(RestTemplate restTemplate,
                                      PlatformProperties platformProperties, DiscoveryClient discoveryClient) {
    ...
    public List<MethodMetaData> fetchServiceMethods(String serviceName, String packageNames) {
        ...
        String url = platformProperties.getFullHttpProtocol() + serviceName + SCAN_URI + params;
        LOGGER.debug("fetch service class methods, service = [{}], url = [{}]", serviceName, url);
        List<MethodMetaData> points;
        try {
            ResponseEntity<List<MethodMetaData>> responseEntity = balanceRestTemplate.exchange(url, HttpMethod.GET, null, METHOD_TYPE);
            ...
            LOGGER.error("fetch service class methods exception, service = {}, package = {}, ex={}", serviceName, packageNames, e.getMessage());
            throw new CommonException("hpfm.warn.fetchMethods.exception", e.getMessage());
        } catch (Exception e) {
            LOGGER.error("fetch service class methods error, service = {}, package = {}, ex = {}", serviceName, packageNames, e.getMessage());
    ...
    public void applyCustomizeRule(String serviceName, List<String> ruleCodes) {
        ...
        for (ServiceInstance instance : instances) {
            ...
            try {
                ResponseEntity<ExceptionResponse> responseEntity = normalRestTemplate.getForEntity(url, ExceptionResponse.class);
                if (!responseEntity.getStatusCode().is2xxSuccessful()) {
                    if (responseEntity.getBody() != null) {
                        throw new CommonException("hpfm.warn.customizeRuleFailure", serviceName, responseEntity.getBody().getMessage());
                    } else {
